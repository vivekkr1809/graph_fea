...
- **API Reference**: Complete documentation for all modules:
  - `mesh` - Mesh generation and edge graph data structures
  - `elements` - CST and GraFEA element implementations
  - `physics` - Material models, damage, and tension-compression split
  - `assembly` - Global matrix assembly and boundary conditions
  - `solvers` - Staggered solver for coupled problems
  - `postprocess` - Visualization and energy tracking utilities
